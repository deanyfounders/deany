...
function ArabiaDiagram({onActivate}){
  ...
  // The markers are now aligned to the visible historical labels:
  // Makkah/Mecca, Yathrib/Medina, Yemen/Sana'a-Marib, Damascus/Syria,
  // the Persian Gulf/Bahrain side, and the Red Sea coast west of Makkah.
  const pts = {
  ...
  const CityPin=({p,primary=false,label,dx=1,dy=-1})=>(
    <g className="node-in">
      <circle cx={p.x} cy={p.y} r={primary?1.35:0.92} fill="rgba(255,255,255,0.92)" stroke={primary?C.gold:C.navy} strokeWidth={primary?0.22:0.16}/>
      <circle cx={p.x} cy={p.y} r={primary?0.58:0.38} fill={primary?C.gold:C.navy}/>
      {primary&&<circle cx={p.x} cy={p.y} r="2.35" fill="none" stroke={C.gold} strokeWidth="0.22" opacity="0.34"/>}
      <text x={p.x+dx} y={p.y+dy} fontSize={primary?1.38:1.08} fontWeight="850" fill={primary?C.gold:rgba(C.navy,0.70)} fontFamily="system-ui,sans-serif" paintOrder="stroke" stroke="rgba(255,255,255,0.88)" strokeWidth="0.42" strokeLinejoin="round">
        {label}
      </text>
    </g>
  );
  ...
  const RouteEndpoint=({p,label,color,align='start',dy=-1})=>(
    <g className="node-in">
      <circle cx={p.x} cy={p.y} r="0.9" fill="rgba(255,255,255,0.90)" stroke={color} strokeWidth="0.22"/>
      <circle cx={p.x} cy={p.y} r="0.38" fill={color}/>
  ...
  return(
    <div style={{borderRadius:24,overflow:'hidden',marginBottom:22,border:`1px solid ${rgba(C.border,0.95)}`,boxShadow:`0 24px 70px ${rgba(C.navy,0.10)}`,background:'white'}}>
      <div style={{background:`linear-gradient(145deg,${rgba(C.teal,0.08)},${rgba(C.gold,0.08)} 55%,${rgba(C.navy,0.04)})`,position:'relative'}}>
        ...
        <div style={{position:'relative',width:'100%',aspectRatio:'1535 / 1024'}}>
          <img
          ...
          <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} preserveAspectRatio="none" style={{position:'absolute',inset:0,width:'100%',height:'100%',display:'block'}}>
            <defs>
              <filter id="routeGlow" x="-20%" y="-20%" width="140%" height="140%">
                <feDropShadow dx="0" dy="0" stdDeviation="0.45" floodColor="#ffffff" floodOpacity="0.90"/>
                <feDropShadow dx="0" dy="0" stdDeviation="0.34" floodColor="#0F172A" floodOpacity="0.14"/>
              </filter>
              <marker id="arrowGold" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="2.3" markerHeight="2.3" orient="auto-start-reverse">
                <path d="M 0 0 L 10 5 L 0 10 z" fill={C.gold}/>
              </marker>
              <marker id="arrowTeal" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="2.3" markerHeight="2.3" orient="auto-start-reverse">
            ...
            {step===0&&(
              <>
                <path d={hijazCorridor} fill="none" stroke="rgba(255,255,255,0.72)" strokeWidth="4.6" strokeLinecap="round"/>
                <path d={hijazCorridor} fill="none" stroke={rgba(C.teal,0.32)} strokeWidth="3.35" strokeLinecap="round"/>
                <path d={hijazCorridor} fill="none" stroke={rgba(C.teal,0.72)} strokeWidth="0.42" strokeLinecap="round" strokeDasharray="1.8 1.4"/>
                <g className="node-in">
            ...
            {step>=1&&(
              <>
                <path d={northSouthRoute} fill="none" stroke="rgba(255,255,255,0.72)" strokeWidth="1.25" strokeLinecap="round"/>
                <path d={northSouthRoute} fill="none" stroke={C.gold} strokeWidth="0.48" strokeLinecap="round" strokeDasharray="220" markerEnd="url(#arrowGold)" filter="url(#routeGlow)" className="draw-line"/>
            ...
            {step>=2&&(
              <>
                <path d={eastWestRoute} fill="none" stroke="rgba(255,255,255,0.72)" strokeWidth="1.25" strokeLinecap="round"/>
                <path d={eastWestRoute} fill="none" stroke={C.teal} strokeWidth="0.48" strokeLinecap="round" strokeDasharray="220" markerEnd="url(#arrowTeal)" filter="url(#routeGlow)" className="draw-line" style={{animationDelay:'0.1s'}}/>
            ...
            {step>=3&&(
              <>
                <circle cx={pts.makkah.x} cy={pts.makkah.y} r="3.1" fill={rgba(C.gold,0.10)} />
                <circle cx={pts.makkah.x} cy={pts.makkah.y} r="4.3" fill="none" stroke={rgba(C.gold,0.24)} strokeWidth="0.34" />
                <g className="node-in">
                  <rect x="35.1" y="53.0" width="13.6" height="4.5" rx="1.9" fill="rgba(15,23,42,0.78)" stroke="rgba(255,255,255,0.45)" strokeWidth="0.12"/>
                  <text x="36.8" y="55.85" fontSize="1.18" fontWeight="900" fill="#ffffff" fontFamily="system-ui,sans-serif">crossroads</text>
                </g>
              </>
            )}
          </svg>
...
function TribalDiagram({onActivate}){
  ...
  return(
    <div style={{borderRadius:24,overflow:'hidden',marginBottom:22,border:`1px solid ${rgba(C.border,0.95)}`,boxShadow:`0 24px 70px ${rgba(C.navy,0.10)}`,background:'white'}}>
      {/* STAGE */}
      <div style={{background:`linear-gradient(145deg,${rgba(C.teal,0.07)},${rgba(C.gold,0.08)} 60%,${rgba(C.navy,0.03)})`}}>
...
function SectionCard({section,onContinue}){
  ...
  return(
    <div className="su rounded-[28px] overflow-hidden" style={{background:'rgba(255,255,255,0.88)',backdropFilter:'blur(18px)',border:focused?`1.5px solid ${rgba(accent,0.55)}`:`1px solid ${rgba(C.border,0.9)}`,transition:'border-color 0.45s ease, box-shadow 0.45s ease, transform 0.45s ease',boxShadow:focused?`0 0 0 4px ${rgba(accent,0.08)}, 0 28px 80px ${rgba(C.navy,0.14)}`:`0 20px 60px ${rgba(C.navy,0.09)}`}}>

      {/* HEADER */}
      <div className="px-7 pt-7 pb-6" style={{borderBottom:`1px solid ${rgba(C.border,0.75)}`,background:`linear-gradient(135deg,${rgba(accent,0.08)},rgba(255,255,255,0.20))`,transition:'opacity 0.5s ease, filter 0.5s ease',...(focused?{opacity:0.25,filter:'blur(1.5px)',pointerEvents:'none'}:{})}}>
...
function QuestionCard({q,qNum,totalQ,onAnswer,onNext}){
  ...
  return(
    <div className="su rounded-[28px] overflow-hidden" style={{background:'rgba(255,255,255,0.88)',backdropFilter:'blur(18px)',border:`1px solid ${rgba(C.border,0.9)}`,boxShadow:`0 20px 60px ${rgba(C.navy,0.09)}`}}>
      <div className="p-7">
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-5 py-12">
        {onBack && onHome && (
          <div className="flex justify-between items-center mb-6">
            <button onClick={onBack} className="flex items-center gap-1.5 text-sm font-medium" style={{color:C.mid,background:'none',border:'none',cursor:'pointer'}} onMouseEnter={e=>e.currentTarget.style.color=C.navy} onMouseLeave={e=>e.currentTarget.style.color=C.mid}>
              <span>← Back</span>
            </button>
            <button onClick={onHome} className="flex items-center gap-1.5 text-white px-4 py-2 rounded-xl text-sm font-semibold shadow-md" style={{background:C.gold,border:'none',cursor:'pointer'}}>
              Home
            </button>
          </div>
        )}
        <div className="text-center mb-9 su">
          ...
          <h1 className="text-5xl font-black mb-3" style={{color:C.navy,fontFamily:'Georgia,serif',letterSpacing:'-0.055em'}}>The Land and Its People</h1>
      ...
      <div className="min-h-screen relative overflow-hidden" style={PAGE_BG}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-5 py-12">
          <div className="rounded-[32px] p-8 border text-center su" style={CARD}>
            ...
            <button onClick={onHome} className="w-full py-3.5 rounded-xl font-bold text-sm pls"
              style={{background:`linear-gradient(135deg,${C.gold},${C.orange})`,color:C.navy,border:'none',cursor:'pointer'}}>
              Next Lesson <ArrowRight className="w-4 h-4 inline ml-1 -mt-0.5"/>
            </button>
            {onHome && <button onClick={onHome} className="w-full py-2.5 rounded-xl text-sm font-medium mt-3"
              style={{background:'transparent',border:`1px solid ${C.border}`,color:C.mid,cursor:'pointer'}}>
              Back to Home
            </button>}
          </div>
        </div>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-5 py-8">
        <div className="sticky top-4 z-30 flex items-center justify-between mb-5 rounded-2xl px-4 py-3" style={{background:'rgba(255,255,255,0.74)',backdropFilter:'blur(16px)',border:`1px solid ${rgba(C.border,0.86)}`,boxShadow:`0 16px 40px ${rgba(C.navy,0.08)}`}}>
          <div className="flex items-center gap-2">
            {onBack && <button onClick={onBack} style={{background:'none',border:'none',cursor:'pointer',color:C.mid,fontSize:14,padding:'2px 4px',display:'flex',alignItems:'center'}} title="Back">←</button>}
