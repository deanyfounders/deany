...
const App = () => {
  ...
  // Auto-skip concept intro if the current lesson/module has no concept cards
  useEffect(() => {
    if (!showConceptIntro || screen !== 'quiz') return;
    const cards = selectedLesson?.conceptCards || selectedModule?.conceptCards;
    if (!cards?.length) { setShowConceptIntro(false); }
  }, [showConceptIntro, screen, selectedLesson, selectedModule]);
  ...
  lessons: [
    ...
    { id: 'lesson-2-1', title: "Valid Sale: Pillars & Conditions", description: "The five pillars every sale needs", duration: "12 min",
      conceptCards: [
        ...
        { title: "Why pillars matter", text: "Think of buying a car: you need to know the exact car (specified), agree the exact price (known), actually hand it over (deliverable), and both genuinely agree (consent).\n\nA 'free' app that hides a mandatory AED 49.99 activation fee? The price was unknown at agreement — that's gharar in the pricing. A gym membership sold as 'transferable' when the gym's terms say otherwise? The subject was never deliverable.\n\nThese aren't edge cases. They're everyday transactions that fail on basic pillars." },
        { title: "The substance test", text: "Islamic commercial law cares about substance over labels. What something IS matters more than what it's CALLED.\n\nA contract labelled 'sale' that functions like an interest-bearing loan fails on substance. A 'free trial' that auto-charges without clear disclosure has a hidden price. A seller who resets odometers hits TWO pillars: misspecified subject AND compromised consent.\n\nYour job in this lesson: learn to spot which pillar is broken, regardless of what the seller calls the deal." }
      ],
      questions: [
        // --- ZONE 1: WARM-UP (Q1-3) ---
    ...
    { id: 'lesson-2-2', title: "Ownership & Possession", description: "Don't sell what you don't own", duration: "12 min",
      conceptCards: [
        { title: "The ownership rule", text: "\"Don't sell what you don't own or possess\" — this hadith establishes a structural rule, not a guideline. Before you can sell something, you must either own it or have constructive possession of it.\n\nThis rule exists to protect buyers. If you sell something you don't control, the buyer's money is at risk and the transaction becomes speculative." },
        { title: "Constructive possession", text: "You don't need to physically hold every item to possess it. Constructive possession (qabd) means:\n\n• Legal control over the asset\n• Payment completed\n• Risk of loss transferred to you\n• Title or documentation in your name\n\nThe key test: if the asset were destroyed right now, who takes the financial hit? If it's you, you have possession. A distributor with paid-for goods insured under her name in a warehouse has constructive possession — even before pickup." },
        { title: "Exceptions & modern applications", text: "Salam (prepaid forward sale) is the structured EXCEPTION to this rule. It has strict safeguards: full prepayment, detailed specifications, and a fixed delivery date. Without these guardrails, selling before owning is prohibited.\n\nModern examples: a dropshipper who lists products they've never touched is selling before owning. An Instagram reseller who takes payment then orders from the supplier has a structural defect. The fix options include: holding inventory, acting as an agent (wakāla), or structuring as proper salam." }
      ],
      questions: [
        // --- ZONE 1: WARM-UP (Q1-3) ---
    ...
    { id: 'lesson-2-3', title: "Risk Before Profit", description: "No risk = no legitimate return", duration: "12 min",
      conceptCards: [
        { title: "The risk–return principle", text: "al-ghurm bil-ghunm / al-kharāj bil-ḍamān: entitlement to gain follows exposure to loss.\n\nThis is the backbone of Islamic commercial law. No risk carried = no legitimate return. If your 'profit' comes without any skin in the game, it resembles ribā.\n\nThis shows up in every contract: the bank in murābaḥa earns profit because it owned the asset and bore the risk. A lessor earns rent because they own the property and bear structural risks." },
        { title: "Who pays for what?", text: "In ijārah (lease), risk allocation is clear:\n\n• The owner/lessor bears: ownership risks, major maintenance, insurance, structural defects from normal wear\n• The lessee bears: operating costs, usage fees, misuse liability, consumables\n\nThink of leasing a car: transmission fails from normal wear = lessor pays. You crash it into a wall = you pay. The principle is simple: risk follows ownership, usage costs follow the user." },
        { title: "Spotting fake risk-sharing", text: "Many products claim to share risk but actually don't. The test: if the investment fails, who actually loses money?\n\n• 'Guaranteed 4% monthly' = no real risk for the investor = ribā territory\n• 'We invest on your behalf with guaranteed outcome' = not valid muḍārabah\n• 'Principal protected plus profit share' = the investor never bears loss = structurally a deposit with interest\n\nReal risk-sharing means both parties can genuinely lose. If one side's outcome is guaranteed, the structure has a ribā defect." }
      ],
      questions: [
        // --- ZONE 1: WARM-UP (Q1-3) ---
    ...
    { id: 'lesson-2-4', title: "Ribawi vs Non-Ribawi Exchanges", description: "Know the matrix", duration: "12 min",
      conceptCards: [
        { title: "What are ribawī items?", text: "Ribawī items are commodities that have special exchange rules to prevent ribā. The classical categories include:\n\n• Gold and silver (and by extension, all currencies)\n• Staple foods: wheat, barley, dates, salt\n\nThese items have stricter rules because they function as stores of value or essential commodities. When you exchange ribawī items, additional safeguards apply that don't apply to regular goods like phones, cars, or furniture." },
        { title: "The exchange rules", text: "Same-kind exchange (e.g. gold for gold, USD for USD):\n→ Must be equal in quantity AND delivered on the spot\n→ Any surplus = ribā al-faḍl. Any delay = ribā al-nasīʾah\n\nDifferent-kind, same-category exchange (e.g. gold for silver, USD for AED):\n→ Amount can differ, BUT delivery must still be on the spot\n→ This is ṣarf (currency exchange) — both currencies must change hands immediately\n\nNon-ribawī items (phones, cars, etc.):\n→ Normal sale rules apply — no special ribawī restrictions" },
        { title: "Why this matters today", text: "Every currency exchange, gold transaction, and remittance you make involves ribawī rules.\n\nA remittance app that takes 3 days to deliver the foreign currency? That's a potential ribā al-nasīʾah issue. A gold trading platform where you never actually own the gold? Multiple defects stacking up.\n\nThe ribawī matrix is your diagnostic tool: classify the items → apply the correct rules → spot the defect. Once you learn this, you can audit any financial product." }
      ],
      questions: [
        // --- ZONE 1: WARM-UP (Q1-3) ---
    ...
    { id: 'lesson-2-5', title: "Price, Time & Credit Sales", description: "Deferred payment ≠ interest", duration: "12 min",
      conceptCards: [
        { title: "Credit sales are permissible", text: "It is permissible to sell an item today at a higher fixed price payable later (deferred/credit sale), as long as the total price is fixed at contract.\n\nA higher credit price is compensation for deferred payment within a SALE — not interest on a loan. The distinction: ribā attaches to MONEY (loans). Credit markup attaches to an ASSET (sale).\n\nThink of it this way: the seller owns a real asset and agrees to sell it at a specific price. The buyer agrees to pay that price over time. No loan was made. An asset changed hands." },
        { title: "The rules that keep it clean", text: "Three critical rules for valid credit sales:\n\n1. ONE fixed price agreed at contract — not 'cash OR credit, decide later'\n2. No increasing the amount after default — that turns unpaid debt into a ribā-generating instrument\n3. The price must be fixed, not floating or indexed to a benchmark rate\n\nA store that says 'Cash AED 5,000 / Credit AED 5,800 — choose AFTER delivery' has a gharar defect: price undefined at contract. Both options can be OFFERED, but one must be CHOSEN and agreed before the deal is done." },
        { title: "Credit markup vs interest", text: "This is the single most asked question: 'Isn't the extra amount just interest with a different name?'\n\nThe structural answer: remove the asset and see what's left. If you strip away the furniture, the phone, the car — and all that remains is 'give me money now, I'll give you more money later' — that's ribā. But when a real asset changes hands at an agreed price, payable over time, that's a sale.\n\nThe test is always structural, not cosmetic. Credit sale: asset + fixed price + time. Ribā: money + surplus + time. The asset is what makes the difference." }
      ],
      questions: [
        // --- ZONE 1: WARM-UP (Q1-3) ---
  ...
  const initCol = (q) => {
    ...
    // Derive items from visual.chips (drag-alloc), columns.correct, or q.items
    const items = q.items
  ...
  const submitCol = () => {
    const q = (selectedLesson?.questions || selectedModule.questions)[currentQuestion];
    const state = initCol(q);
    const totalItems = q.items?.length || q.visual?.chips?.length || q.columns.reduce((s, c) => s + (c.correct?.length || 0), 0);
    const totalPlaced = Object.values(state.columns).reduce((s, arr) => s + arr.length, 0);
    if (totalPlaced < totalItems) return;
  ...
  if (screen == 'quiz' && (selectedModule || selectedLesson)) {
    ...
    if (showConceptIntro) {
      const cards = selectedLesson?.conceptCards || selectedModule?.conceptCards;
      if (!cards?.length) { /* no concept cards — fall through to quiz render below */ }
      else {
